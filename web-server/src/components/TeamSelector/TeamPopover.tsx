              ...
              {!isSingleMode && (
                <Box display="flex" justifyContent="space-between" width="100%">
                  <Button
                    ...
                    sx={{ fontSize: 'smaller' }}
                    onClick={() =>
                      dispatch(appSlice.actions.setMultiTeam(apiTeams))
                    }
                  >
                    Select All
                  </Button>
                  <Button
                    ...
                    color="secondary"
                    sx={{ fontSize: 'smaller' }}
                    onClick={() => dispatch(appSlice.actions.setMultiTeam([]))}
                  >
                    Unselect All
                  </Button>
                </Box>
                ...
                {apiTeams.length ? (
                  <>
                    ...
                    {listFilteredBySearch.map((apiTeam) => {
                      ...
                      const managerName =
                        apiTeam.manager_id &&
                        usersMap[apiTeam.manager_id]?.name;
                      ...
                      return (
                        <MenuItem
                          ...
                          onClick={() => {
                            if (isSingleMode) {
                              dispatch(
                                appSlice.actions.setSingleTeam([apiTeam])
                              );
                              setProdBranchNamesByTeamId(apiTeam.id);
                              track(activeRouteEvent, { team: apiTeam });
                            } else {
                              dispatch(
                                appSlice.actions.toggleMultiTeam(apiTeam)
                              );
                              track('APP_TEAM_TOGGLE_MULTI', {
                                team: apiTeam
                              });
                            }
                            if (closeOnSelect) {
                          ...
                          >
                            <Box overflow="hidden">
                              ...
                              <Typography variant="subtitle1">
                                {apiTeam.member_ids?.length}{' '}
                                {pluralize(
                                  'member',
                                  apiTeam.member_ids?.length || 0
                                )}
                              </Typography>
                            </Box>
                            <Box textAlign="right">
                              <Box
                                lineHeight={theme.spacing(
                                  1.8 *
                                    1.5 /* team font size * its line height */
                                )}
                              >
                                Manager
                              </Box>
